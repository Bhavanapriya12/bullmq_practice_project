...
module.exports = {
  ...
  find_one: async (collection, condition, select, sort) => {
    console.log("Find one function called --->", eval(collection));
    return await eval(collection)
      .findOne(condition)
      .select(select)
      .sort(sort)
  ...
  bulkWrite: async (collectionName, operations) => {
    try {
      const result = await eval(collectionName).bulkWrite(operations);
      return result;
    } catch (error) {
      throw new Error(`Bulk write error: ${error.message}`);
    }
  },
